 ...
 */
public class ImgCropHandleUtil {
     ...
     *
     * @return the ImgCropHandle that was pressed; null if no ImgCropHandle was pressed
     */
    public static ImgCropHandle getPressedHandle(float x,
                                                 float y,
                                                 float left,
                                                 float top,
                                                 float right,
        ...
        ImgCropHandle closestHandle = null;
        float closestDistance = Float.POSITIVE_INFINITY;

        final float distanceToTopLeft = ImgCropMathUtil.calculateDistance(x, y, left, top);
        if (distanceToTopLeft < closestDistance) {
            closestDistance = distanceToTopLeft;
            closestHandle = ImgCropHandle.TOP_LEFT;
        }

        final float distanceToTopRight = ImgCropMathUtil.calculateDistance(x, y, right, top);
        if (distanceToTopRight < closestDistance) {
            closestDistance = distanceToTopRight;
            closestHandle = ImgCropHandle.TOP_RIGHT;
        }

        final float distanceToBottomLeft = ImgCropMathUtil.calculateDistance(x, y, left, bottom);
        if (distanceToBottomLeft < closestDistance) {
            closestDistance = distanceToBottomLeft;
            closestHandle = ImgCropHandle.BOTTOM_LEFT;
        }

        final float distanceToBottomRight = ImgCropMathUtil.calculateDistance(x, y, right, bottom);
        if (distanceToBottomRight < closestDistance) {
            closestDistance = distanceToBottomRight;
            closestHandle = ImgCropHandle.BOTTOM_RIGHT;
        }
        ...
        if (ImgCropHandleUtil.isInHorizontalTargetZone(x, y, left, right, top, targetRadius)) {
            return ImgCropHandle.TOP;
        } else if (ImgCropHandleUtil.isInHorizontalTargetZone(x, y, left, right, bottom, targetRadius)) {
            return ImgCropHandle.BOTTOM;
        } else if (ImgCropHandleUtil.isInVerticalTargetZone(x, y, left, top, bottom, targetRadius)) {
            return ImgCropHandle.LEFT;
        } else if (ImgCropHandleUtil.isInVerticalTargetZone(x, y, right, top, bottom, targetRadius)) {
            return ImgCropHandle.RIGHT;
        }
     ...
     */
    public static void getOffset(@NonNull ImgCropHandle handle,
                                 float x,
                                 float y,
                                 float left,
                                 float top,
