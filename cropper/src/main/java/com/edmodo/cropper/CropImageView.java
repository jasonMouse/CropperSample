...
import com.edmodo.cropper.cropwindow.handle.ImgCropHandle;
import com.edmodo.cropper.util.ImgCropAspectRatioUtil;
import com.edmodo.cropper.util.ImgCropHandleUtil;
import com.edmodo.cropper.util.ImgCropPaintUtil;
...
public class CropImageView extends ImageView {
    ...
    // The ImgCropHandle that is currently pressed; null if no ImgCropHandle is pressed.
    private ImgCropHandle mPressedHandle;
    ...
    private void init(@NonNull Context context, @Nullable AttributeSet attrs) {
        ...
        mBorderThickness = typedArray.getDimension(R.styleable.CropImageView_borderWidth, resources.getDimension(R.dimen.border_thickness));
        int borderColor = typedArray.getColor(R.styleable.CropImageView_borderColor, resources.getColor(R.color.border));
        mCornerThickness = typedArray.getDimension(R.styleable.CropImageView_cornerWidth, resources.getDimension(R.dimen.corner_thickness));
        mCornerLength = typedArray.getDimension(R.styleable.CropImageView_cornerLength, resources.getDimension(R.dimen.corner_length));
        int cornerColor = typedArray.getColor(R.styleable.CropImageView_cornerColor, resources.getColor(R.color.corner));

        float guidelineWidth = typedArray.getDimension(R.styleable.CropImageView_guidelineWidth, resources.getDimension(R.dimen.guideline_thickness));
        int guidelineColor = typedArray.getColor(R.styleable.CropImageView_guidelineColor, resources.getColor(R.color.guideline));
        int surroundingAreaBg = typedArray.getColor(R.styleable.CropImageView_surroundingAreaBg, resources.getColor(R.color.surrounding_area));
        typedArray.recycle();

        mBorderPaint = ImgCropPaintUtil.newBorderPaint(mBorderThickness, borderColor);
        mGuidelinePaint = ImgCropPaintUtil.newGuidelinePaint(guidelineWidth, guidelineColor);
        mSurroundingAreaOverlayPaint = ImgCropPaintUtil.newSurroundingAreaOverlayPaint(surroundingAreaBg);
        mCornerPaint = ImgCropPaintUtil.newCornerPaint(mCornerThickness, cornerColor);
    }
     ...
     * @return
     */
    public Bitmap getCroppedResult() {
        ...
        Bitmap result = null;
        // 如果当前图片是平铺展开显示，那么直接执行裁剪即可
        if (bitmapWidth == viewWidth && bitmapHeight == viewHeight) {
            result = getCroppedImage(0, 0, originalBitmap);
        } else {
            // 否则需要手动计算缩放比例
            float scaleX = ((float) viewWidth) / bitmapWidth;
            float scaleY = ((float) viewHeight) / bitmapHeight;
            result = getCroppedImage(scaleX, scaleY, originalBitmap);
        }
        return result;
    }
     ...
     */
    private Bitmap getCroppedImage(final float scaleSourceX, final float scaleSourceY, final Bitmap originalBitmap) {
        ...
        final float scaleX = scaleSourceX > 0 ? scaleSourceX : matrixValues[Matrix.MSCALE_X];
        final float scaleY = scaleSourceY > 0 ? scaleSourceY : matrixValues[Matrix.MSCALE_Y];
        ...
        final float cropX = (bitmapLeft + ImgCropEdge.LEFT.getCoordinate()) / scaleX;
        final float cropY = (bitmapTop + ImgCropEdge.TOP.getCoordinate()) / scaleY;
        ...
        final float cropWidth = Math.min(ImgCropEdge.getWidth() / scaleX, originalBitmap.getWidth() - cropX);
        final float cropHeight = Math.min(ImgCropEdge.getHeight() / scaleY, originalBitmap.getHeight() - cropY);
    ...
    /**
     * Initialize the crop window by setting the proper {@link ImgCropEdge} values.
     * <p/>
    ...
    private void initCropWindow(@NonNull RectF bitmapRect) {
        ...
        if (mFixAspectRatio) {
            ...
            ImgCropEdge.LEFT.setCoordinate(bitmapRect.left + horizontalPadding);
            ImgCropEdge.TOP.setCoordinate(bitmapRect.top + verticalPadding);
            ImgCropEdge.RIGHT.setCoordinate(bitmapRect.right - horizontalPadding);
            ImgCropEdge.BOTTOM.setCoordinate(bitmapRect.bottom - verticalPadding);
        }
    }
    ...
    private void initCropWindowWithFixedAspectRatio(@NonNull RectF bitmapRect) {
        ...
        if (ImgCropAspectRatioUtil.calculateAspectRatio(bitmapRect) > getTargetAspectRatio()) {

            final float cropWidth = ImgCropAspectRatioUtil.calculateWidth(bitmapRect.height(), getTargetAspectRatio());

            ImgCropEdge.LEFT.setCoordinate(bitmapRect.centerX() - cropWidth / 2f);
            ImgCropEdge.TOP.setCoordinate(bitmapRect.top);
            ImgCropEdge.RIGHT.setCoordinate(bitmapRect.centerX() + cropWidth / 2f);
            ImgCropEdge.BOTTOM.setCoordinate(bitmapRect.bottom);

        } else {

            final float cropHeight = ImgCropAspectRatioUtil.calculateHeight(bitmapRect.width(), getTargetAspectRatio());

            ImgCropEdge.LEFT.setCoordinate(bitmapRect.left);
            ImgCropEdge.TOP.setCoordinate(bitmapRect.centerY() - cropHeight / 2f);
            ImgCropEdge.RIGHT.setCoordinate(bitmapRect.right);
            ImgCropEdge.BOTTOM.setCoordinate(bitmapRect.centerY() + cropHeight / 2f);
        }
    }
    ...
    private void drawDarkenedSurroundingArea(@NonNull Canvas canvas) {
        ...
        final float left = ImgCropEdge.LEFT.getCoordinate();
        final float top = ImgCropEdge.TOP.getCoordinate();
        final float right = ImgCropEdge.RIGHT.getCoordinate();
        final float bottom = ImgCropEdge.BOTTOM.getCoordinate();
    ...
    private void drawGuidelines(@NonNull Canvas canvas) {
        ...
        final float oneThirdCropWidth = ImgCropEdge.getWidth() / 3;
        ...
        final float oneThirdCropHeight = ImgCropEdge.getHeight() / 3;
    ...
    private void drawBorder(@NonNull Canvas canvas) {
        ...
        canvas.drawRect(ImgCropEdge.LEFT.getCoordinate(),
                        ImgCropEdge.TOP.getCoordinate(),
                        ImgCropEdge.RIGHT.getCoordinate(),
                        ImgCropEdge.BOTTOM.getCoordinate(),
                        mBorderPaint);
    }
    ...
    private void onActionDown(float x, float y) {
        ...
        mPressedHandle = ImgCropHandleUtil.getPressedHandle(x, y, left, top, right, bottom, mHandleRadius);
        ...
        if (mPressedHandle != null) {
            ImgCropHandleUtil.getOffset(mPressedHandle, x, y, left, top, right, bottom, mTouchOffset);
            invalidate();
        }
    }
