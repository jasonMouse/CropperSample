...
public enum ImgCropEdge {
    ...
    /**
     * Sets the coordinate of the ImgCropEdge. The coordinate will represent the x-coordinate for LEFT and
    ...
    /**
     * Add the given number of pixels to the current coordinate position of this ImgCropEdge.
     *
    ...
    /**
     * Gets the coordinate of the ImgCropEdge
     *
     * @return the ImgCropEdge coordinate (x-coordinate for LEFT and RIGHT Edges and the y-coordinate for
    ...
    /**
     * Sets the ImgCropEdge to the given x-y coordinate but also adjusting for snapping to the image bounds
    ...
    /**
     * Adjusts this ImgCropEdge position such that the resulting window will have the given aspect ratio.
     *
    ...
    public void adjustCoordinate(float aspectRatio) {
        ...
        switch (this) {
            case LEFT:
                mCoordinate = ImgCropAspectRatioUtil.calculateLeft(top, right, bottom, aspectRatio);
                break;
            case TOP:
                mCoordinate = ImgCropAspectRatioUtil.calculateTop(left, right, bottom, aspectRatio);
                break;
            case RIGHT:
                mCoordinate = ImgCropAspectRatioUtil.calculateRight(left, top, bottom, aspectRatio);
                break;
            case BOTTOM:
                mCoordinate = ImgCropAspectRatioUtil.calculateBottom(left, top, right, aspectRatio);
                break;
        }
    }
     ...
     *
     * @param edge        the ImgCropEdge that is about to be expanded
     ...
     */
    public boolean isNewRectangleOutOfBounds(@NonNull ImgCropEdge edge, @NonNull RectF imageRect, float aspectRatio) {
        ...
        switch (this) {
            ...
            case LEFT:
                ...
                if (edge.equals(ImgCropEdge.TOP)) {

                    final float top = imageRect.top;
                    final float bottom = ImgCropEdge.BOTTOM.getCoordinate() - offset;
                    final float right = ImgCropEdge.RIGHT.getCoordinate();
                    final float left = ImgCropAspectRatioUtil.calculateLeft(top, right, bottom, aspectRatio);

                    return isOutOfBounds(top, left, bottom, right, imageRect);

                } else if (edge.equals(ImgCropEdge.BOTTOM)) {

                    final float bottom = imageRect.bottom;
                    final float top = ImgCropEdge.TOP.getCoordinate() - offset;
                    final float right = ImgCropEdge.RIGHT.getCoordinate();
                    final float left = ImgCropAspectRatioUtil.calculateLeft(top, right, bottom, aspectRatio);
            ...
            case TOP:
                ...
                if (edge.equals(ImgCropEdge.LEFT)) {

                    final float left = imageRect.left;
                    final float right = ImgCropEdge.RIGHT.getCoordinate() - offset;
                    final float bottom = ImgCropEdge.BOTTOM.getCoordinate();
                    final float top = ImgCropAspectRatioUtil.calculateTop(left, right, bottom, aspectRatio);

                    return isOutOfBounds(top, left, bottom, right, imageRect);

                } else if (edge.equals(ImgCropEdge.RIGHT)) {

                    final float right = imageRect.right;
                    final float left = ImgCropEdge.LEFT.getCoordinate() - offset;
                    final float bottom = ImgCropEdge.BOTTOM.getCoordinate();
                    final float top = ImgCropAspectRatioUtil.calculateTop(left, right, bottom, aspectRatio);
            ...
            case RIGHT:
                ...
                if (edge.equals(ImgCropEdge.TOP)) {
                    ...
                    final float bottom = ImgCropEdge.BOTTOM.getCoordinate() - offset;
                    final float left = ImgCropEdge.LEFT.getCoordinate();
                    final float right = ImgCropAspectRatioUtil.calculateRight(left, top, bottom, aspectRatio);
            ...
            case BOTTOM:
                ...
                if (edge.equals(ImgCropEdge.LEFT)) {
                    ...
                    final float right = ImgCropEdge.RIGHT.getCoordinate() - offset;
                    final float top = ImgCropEdge.TOP.getCoordinate();
                    final float bottom = ImgCropAspectRatioUtil.calculateBottom(left, top, right, aspectRatio);
    ...
    /**
     * Snap this ImgCropEdge to the given image boundaries.
     *
    ...
    public static float getWidth() {
        return ImgCropEdge.RIGHT.getCoordinate() - ImgCropEdge.LEFT.getCoordinate();
    }
    ...
    public static float getHeight() {
        return ImgCropEdge.BOTTOM.getCoordinate() - ImgCropEdge.TOP.getCoordinate();
    }

    /**
     * Determines if this ImgCropEdge is outside the inner margins of the given bounding rectangle. The
    ...
    private static float adjustLeft(float x, @NonNull RectF imageRect, float imageSnapRadius, float aspectRatio) {
        ...
        if (x - imageRect.left < imageSnapRadius) {
            ...
            if (x >= ImgCropEdge.RIGHT.getCoordinate() - MIN_CROP_LENGTH_PX) {
                resultXHoriz = ImgCropEdge.RIGHT.getCoordinate() - MIN_CROP_LENGTH_PX;
            }
            // Checks if the window is too small vertically
            if (((ImgCropEdge.RIGHT.getCoordinate() - x) / aspectRatio) <= MIN_CROP_LENGTH_PX) {
                resultXVert = ImgCropEdge.RIGHT.getCoordinate() - (MIN_CROP_LENGTH_PX * aspectRatio);
            }
    ...
    private static float adjustRight(float x, @NonNull RectF imageRect, float imageSnapRadius, float aspectRatio) {
        ...
        if (imageRect.right - x < imageSnapRadius) {
            ...
            // Checks if the window is too small horizontally
            if (x <= ImgCropEdge.LEFT.getCoordinate() + MIN_CROP_LENGTH_PX) {
                resultXHoriz = ImgCropEdge.LEFT.getCoordinate() + MIN_CROP_LENGTH_PX;
            }
            // Checks if the window is too small vertically
            if (((x - ImgCropEdge.LEFT.getCoordinate()) / aspectRatio) <= MIN_CROP_LENGTH_PX) {
                resultXVert = ImgCropEdge.LEFT.getCoordinate() + (MIN_CROP_LENGTH_PX * aspectRatio);
            }
    ...
    private static float adjustTop(float y, @NonNull RectF imageRect, float imageSnapRadius, float aspectRatio) {
        ...
        if (y - imageRect.top < imageSnapRadius) {
            ...
            // Checks if the window is too small vertically
            if (y >= ImgCropEdge.BOTTOM.getCoordinate() - MIN_CROP_LENGTH_PX)
                resultYHoriz = ImgCropEdge.BOTTOM.getCoordinate() - MIN_CROP_LENGTH_PX;

            // Checks if the window is too small horizontally
            if (((ImgCropEdge.BOTTOM.getCoordinate() - y) * aspectRatio) <= MIN_CROP_LENGTH_PX)
                resultYVert = ImgCropEdge.BOTTOM.getCoordinate() - (MIN_CROP_LENGTH_PX / aspectRatio);
    ...
    private static float adjustBottom(float y, @NonNull RectF imageRect, float imageSnapRadius, float aspectRatio) {
        ...
        if (imageRect.bottom - y < imageSnapRadius) {
            ...
            // Checks if the window is too small vertically
            if (y <= ImgCropEdge.TOP.getCoordinate() + MIN_CROP_LENGTH_PX) {
                resultYVert = ImgCropEdge.TOP.getCoordinate() + MIN_CROP_LENGTH_PX;
            }
            // Checks if the window is too small horizontally
            if (((y - ImgCropEdge.TOP.getCoordinate()) * aspectRatio) <= MIN_CROP_LENGTH_PX) {
                resultYHoriz = ImgCropEdge.TOP.getCoordinate() + (MIN_CROP_LENGTH_PX / aspectRatio);
            }
